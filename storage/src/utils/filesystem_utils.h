 ...
 * @param logical_block_path Puntero al buffer donde se guardará la ruta
 * completa.
 ...
 * @param physical_block_path Puntero al buffer donde se guardará la ruta
 * completa.
 ...
 *
 * @param logical_block_path Ruta completa a un hardlink lógico que apunta al
 * bloque físico.
 * @return int positivo que indica la cantidad de hardlinks del bloque físico,
 * -1 en caso contrario.
 */
 ...
 * La función maneja internamente el bloqueo del mutex global
 * g_storage_bitmap_mutex, pero lo deja desbloqueado en caso de error.
 *
 * @param bitmap Doble puntero a t_bitarray donde se almacenará la estructura.
 * @param bitmap_buffer Doble puntero al char donde se almacenará el buffer
 * crudo.
 ...
 * @return int 0 si la persistencia es exitosa, un valor negativo en caso de
 * error.
 */
 ...
 * @return ssize_t El índice del bit libre encontrado, o -1 si el bitmap está
 * lleno.
 */
...
/**
 * Crea un paquete de respuesta estándar para notificar un error de protocolo al
 * Worker.
 * * El paquete contiene la Query ID, el código de operación (ej.
 * WRITE_BLOCK_RES) y el código de error específico (ej. FILE_TAG_MISSING).
 *
 * @param query_id ID de la Query asociada.
 * @param op_code Código de operación del tipo de respuesta (ej.
 * STORAGE_OP_BLOCK_WRITE_RES).
 * @param op_error_code Código de error específico del Storage.
 * @return t_package* El paquete de respuesta de error creado, o NULL si falla
