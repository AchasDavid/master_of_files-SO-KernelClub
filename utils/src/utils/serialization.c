...
void buffer_write_uint8(t_buffer *buffer, uint8_t value)
{
    ...
    uint8_t *value_first_position = (uint8_t *)buffer->stream + buffer->offset;

    if (remaining_capacity < sizeof(uint8_t))
        return;

    memcpy(value_first_position, &value, sizeof(uint8_t));
...
{
    size_t remaining_capacity = buffer->size - buffer->offset;
    uint16_t *value_first_position = (uint16_t *)buffer->stream + buffer->offset;

    if (remaining_capacity < sizeof(uint16_t))
        return;

    memcpy(value_first_position, &value, sizeof(uint16_t));
...
{
    size_t remaining_capacity = buffer->size - buffer->offset;
    uint32_t *value_first_position = (uint32_t *)buffer->stream + buffer->offset;

    if (remaining_capacity < sizeof(uint32_t))
        return;

    memcpy(value_first_position, &value, sizeof(uint32_t));
...
void buffer_write_string(t_buffer *buffer, char *value)
{
    ...
    size_t remaining_capacity = buffer->size - buffer->offset;
    char *value_first_position = (char *)buffer->stream + buffer->offset;

    if (remaining_capacity < str_length)
        return;

    memcpy(value_first_position, &value, str_length);
